...
EOF
  ...
  local uuid="$(uuidgen)"
  ansi_tmpfile="$(mktemp "/tmp/$name.$(date +'%Y-%m-%d-%H-%M-%S').${uuid}.$(pwd | sed 's:/:_:g').XXXXX.ansi.log")"
  raw_tmpfile="$(mktemp "/tmp/$name.$(date +'%Y-%m-%d-%H-%M-%S').${uuid}.$(pwd | sed 's:/:_:g').XXXXX.raw.log")"
