...
if [ $(uname -s) = "Darwin" ]; then
  ...
  if ! test "x$have_eval_command" = "x"; then
    output=$($GDB -n --batch --eval-command="$backtrace" /proc/$1/exe $1 2>&1 </dev/null)
    if printf '%s' "$output" | grep -q 'Could not attach to process.'; then
      echo -n $'\e[31m'
      printf '%s\n' "$output"
      echo -n $'\e[0m'
      exit 1
    ...
    $GDB -q -n /proc/$1/exe $1 <<EOF 2>&1 | sed -n \
      -e 's/^(gdb) //' \
      -e '/^#/p' \
      -e '/^   /p' \
      -e 's/\(^Thread.*\)/@\1/p' | tr '@' '\n' >$OUTFILE
